package tqdream.base.utils.security;

import java.util.Arrays;

/**
 * @author taoqiang @create on 2016/10/26 17:20
...
public class SMS4 {
    ...
    void SMS4Crypt(byte[] Input, byte[] Output, int[] rk) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            x[i] = tmp[0] << 24 | tmp[1] << 16 | tmp[2] << 8 | tmp[3]; // x[i]=(Input[0+4*i]<<24|Input[1+4*i]<<16|Input[2+4*i]<<8|Input[3+4*i]); }
            for (r = 0; r < 32; r += 4) {
    ...
    private void SMS4KeyExt(byte[] Key, int[] rk, int CryptFlag) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            x[i] = tmp[0] << 24 | tmp[1] << 16 | tmp[2] << 8 | tmp[3]; // x[i]=Key[0+4*i]<<24|Key[1+4*i]<<16|Key[2+4*i]<<8|Key[3+4*i];
        }
        x[0] ^= 0xa3b1bac6;
        x[1] ^= 0x56aa3350;
        x[2] ^= 0x677d9197;
...
//             解密时轮密钥使用顺序：rk31,rk30,...,rk0
    ...
    /** * 不限明文长度的SMS4加密 *  * @param plaintext * @param key * @return */
    ...
    /**
     * 不限明文长度的SMS4解密 *  * @param ciphertext * @param key * @return
     */
    ...
    /**
     * 解密，获得明文字符串 * @param ciphertext * @param key * @return
     */
    ...
    /**
     * 只加密16位明文 *  * @param plaintext * @param key * @return
     */
    ...
    /**
     * 只解密16位密文 *  * @param plaintext * @param key * @return
     */
